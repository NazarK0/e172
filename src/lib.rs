...
impl FrameworkApp {
    ...
    fn draw_recursive<'a>(
        ...
        // Малюємо поточний віджет
        root.draw(engine, render_pass);

        // Рекурсивно малюємо дітей
        for child in root.children() {
            // Викликаємо саму себе як статичну функцію
            Self::draw_recursive(child, engine, render_pass);
        }
    }
...
impl ApplicationHandler for FrameworkApp {
            ...
            winit::event::WindowEvent::RedrawRequested => {
...

                {
                    ...
                    if let Some(root) = &self.root_widget {
                        Self::draw_recursive(root, engine, &mut render_pass);
                    }
                }
